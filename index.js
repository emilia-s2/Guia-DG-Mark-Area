...
/*		if (whichmode== 466 && event.templateId==46602 && event.stage==0) {
		...
		if ([459, 759].includes(whichmode) && [1001, 1004].includes(event.templateId) && event.stage==0) {
			if (event.skill.id==3107) { // 重击
				SpawnThing(event.loc, event.w, 2000, 2,  90, 100,   0, 1000, 170);
				SpawnThing(event.loc, event.w, 2000, 2, 270, 100,   0, 1000, 190);
			}
			if ([1106, 2106].includes(event.skill.id)) { // 旋转攻击
				SpawnThing(event.loc, event.w, 3000, 3,   0,   0,   0,  360,  10, 330);
			}
		}
		// FI_2王 303150
		...
		if ([770, 970].includes(whichmode) && event.templateId==1000 && event.stage==0) {
			if (skillid==107) { // 前喷
				SpawnThing(event.loc, event.w, 3000, 2,   0,   0,   0,  500, 130);
				SpawnThing(event.loc, event.w, 3000, 2,   0,   0,   0,  500, 230);
			}
		}
		// RM_2王 300870
		if ([770, 970].includes(whichmode) && event.templateId==2000 && event.stage==0) {
			if (skillid==111) { // 直线攻击
				SpawnThing(event.loc, event.w, 2000, 2,  90,  80,   0,  500, 180);
				SpawnThing(event.loc, event.w, 2000, 2, 270,  80,   0,  500, 180);
			}
			if (skillid==106) { //插地眩晕
				SpawnThing(event.loc, event.w, 2000, 3, 180,  50,   0,  360,  18, 180);
			}
		}
		// RM_3王 303260
		if ([770, 970].includes(whichmode) && event.templateId==3000 && event.stage==0) {
			if (skillid==106) { // 前推坦
				SpawnThing(event.loc, event.w, 2000, 2,   0,  50,   0,  530, 145);
				SpawnThing(event.loc, event.w, 2000, 2,   0,  50,   0,  530, 250);
				SpawnThing(event.loc, event.w, 2000, 3,   0,  50, 145,  250,   8, 530);
			}
			if (skillid==110) { // 尾巴横扫
				SpawnThing(event.loc, event.w, 2000, 2,   0,   0,   0,  620, 158);
				SpawnThing(event.loc, event.w, 2000, 2,   0,   0,   0,  620, 202);
				SpawnThing(event.loc, event.w, 2000, 3,   0,   0, 202,  158,   4, 620);
			}
			if ([113, 116].includes(skillid)) { // 鉴定 内外圈
				SpawnThing(event.loc, event.w, 3000, 3,   0,   0,   0,  360,  10, 290);
			}
			if (skillid==322) { // 命运圈
				SpawnThing(event.loc, event.w, 6000, 3,   0,   0,   0,  360,  12, 230);
...
//				SpawnPoint(event.loc, event.w, 2000, 1,  90, 250);
		...
		if ([781, 981].includes(whichmode) && event.templateId==3000 && event.stage==0) {
			if (skillid==116 && whichmode==781) { // 下级 前盾砸 132
				SpawnThing(event.loc, event.w, 5000, 2, 180,  50,   0,  500, 115);
				SpawnThing(event.loc, event.w, 5000, 2, 180,  50,   0,  500, 245);
			} 
			if (skillid==116 && whichmode==981) { // 上级 甜甜圈 140
				SpawnThing(event.loc, event.w, 8000, 3, 180,  35,   0,  360,  18, 200);
				SpawnThing(event.loc, event.w, 8000, 3, 180,  35,   0,  360,  10, 360);
...
/*		if ([783, 983, 3018].includes(whichmode) && event.templateId==1000 && event.stage==0) {
			...
			if (skillid==119) { // 蓄力捶地
				SpawnThing(event.loc, event.w, 2000, 3,   0,  90,   0,  360,   8, 420);
			}
		}
		// DRC_2王 304080
		if ([783, 983, 3018].includes(whichmode) && event.templateId==2000 && event.stage==0) {
			if (skillid==105) { // 点名(击飞)
				SpawnThing(event.loc, event.w, 3000, 2,   0,   0,   0,  600, 180);
			}
		...
		if ([783, 983, 3018].includes(whichmode) && event.templateId==3000 && event.stage==0) {
			if (skillid==303) { // S攻击 右
				SpawnThing(event.loc, event.w, 5000, 2,   0,   0,   0,  400,  90);
				SpawnThing(event.loc, event.w, 5000, 2,   0,   0,   0,  400, 270);
				SpawnPoint(event.loc, event.w, 2000, 1,  80, 250);
				SpawnPoint(event.loc, event.w, 2000, 1, 260, 250);
			}
		...
		if ([782, 982, 3019].includes(whichmode) && event.templateId==1000 && event.stage==0) {
			if (skillid==107) { // 后喷
				SpawnThing(event.loc, event.w, 3000, 2,   0,   0,   0,  500,  45);
				SpawnThing(event.loc, event.w, 3000, 2,   0,   0,   0,  500, 315);
			}
		}
		// GLS_2王 304060
...
//				SpawnThing(event.loc, event.w, 5000, 3,   0,   0,   0,  360,   4, 680);   //
		...
		if ([782, 982, 3019].includes(whichmode) && event.templateId==3000) {
			if ([146, 154].includes(skillid)) { // 右扩散电圈标记
				SpawnPoint(event.loc, event.w, 4000, 1, 325, 370);
			}
...
//				SpawnThing(event.loc, event.w, 4000, 3, 180,  88,   0,  360,  10, 300);  //
		...
		if ([3101, 3201].includes(whichmode) && event.templateId==2000 && event.stage==0) {
			if (skillid==108) { // 前插 后喷
				SpawnThing(event.loc, event.w, 4000, 2,  90,  80,   0, 1000,  17);
				SpawnThing(event.loc, event.w, 4000, 2, 270,  80,   0, 1000, 343);
			}
			if (skillid==236) { // 点名 前推
				SpawnThing(event.loc, event.w, 4000, 2,  90,  80,   0, 1000, 163);
